...
#define TIME_REMAINING 2.0
#define AUTHOR_SIZE 1.0
...
e_window_main_loop_fn e_r_startup(const char *author, ivec2 wnd_size, on_startup_finished_fun on_startup_finished) {
    ...
    singleton.time_remaining = TIME_REMAINING;
...
static void main_loop(float delta_time) {
    singleton.time_remaining -= delta_time;
    if(singleton.time_remaining <= 0)
